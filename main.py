...
def safe_api_call(url, payload, method="POST", timeout=300):
    """지수 백오프를 적용한 안전한 API 호출 함수 (1s, 2s, 4s, 8s, 16s)"""
    delays = [1, 2, 4, 8, 16]
...
class VersatileKeywordEngine:
    def __init__(self, api_key):
        ...
        self.model = "gemini-1.5-flash" # 또는 gemini-flash-latest
        self.categories = {
...
- 분량: 2,500~3,000자 내외.
- 날짜 정보 배제: 연도, 월, 일 정보를 제목과 본문에 포함하지 마세요.
...
def main():
    ...
    if not data: 
        print("❌ 콘텐츠 생성 단계에서 실패했습니다 (API 할당량 초과 가능성).")
        return
